...
stream::stream(uv_stream_t* stream) :
    handle(reinterpret_cast<uv_handle_t*>(stream)), stream_(stream),
    on_connection_(), on_read_(), on_complete_() {
  assert(stream_);
}

void stream::on_read(on_read_callback_type callback) {
  on_read_ = callback;
}

void stream::on_complete(on_complete_callback_type callback) {
  on_complete_ = callback;
}

void stream::on_connection(on_connection_callback_type callback) {
  on_connection_ = callback;
}

void stream::set_handle(uv_handle_t* h) {
  handle::set_handle(h);
  stream_ = reinterpret_cast<uv_stream_t*>(h);
}

std::size_t stream::write_queue_size() const {
  if (stream_)
    return stream_->write_queue_size;
  else
    return 0;
}

resval stream::read_start() {
  bool ipc_pipe = stream_->type == UV_NAMED_PIPE
      && reinterpret_cast<uv_pipe_t*>(stream_)->ipc;

  if (ipc_pipe) {
    return run_(uv_read2_start, stream_, on_alloc,
        [](uv_pipe_t* handle, ssize_t nread, uv_buf_t buf,
            uv_handle_type pending) {
          auto self = reinterpret_cast<stream*>(handle->data);
          assert(self);
          self->after_read_(reinterpret_cast<uv_stream_t*>(handle), nread, buf,
              pending);
        });
  } else {
    return run_(uv_read_start, stream_, on_alloc,
        [](uv_stream_t* handle, ssize_t nread, uv_buf_t buf) {
          auto self = reinterpret_cast<stream*>(handle->data);
          assert(self);
...
resval stream::listen(int backlog) {
  return run_(uv_listen, uv_stream(), backlog,
...
void stream::after_read_(uv_stream_t* handle, ssize_t nread, uv_buf_t buf,
  ...
  if (nread < 0) {
    ...
    if (on_read_)
      on_read_(nullptr, 0, 0, nullptr, get_last_error());
  } else {
    ...
    if (nread > 0) {
      ...
      if (pending == UV_TCP) {
        ...
        if (on_read_)
          on_read_(buf.base, 0, nread, accepted, resval());
      } else {
        // Only TCP supported
        ...
        // invoke "onread" callback
        if (on_read_)
          on_read_(buf.base, 0, nread, nullptr, resval());
      }
    }
  }
