...
namespace native
{
    namespace detail
    {
        ...
        resval stream::write(const char* data, int offset, int length, stream* send_stream)
        {
            bool res = false;
            bool ipc_pipe = stream_->type == UV_NAMED_PIPE && reinterpret_cast<uv_pipe_t*>(stream_)->ipc;
        ...
        void stream::after_read_(uv_stream_t* handle, ssize_t nread, uv_buf_t buf, uv_handle_type pending)
        {
            if(nread < 0)
            {
