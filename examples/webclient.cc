...
int main(int argc, char** argv) {
    return run([=]() {
		...
		req->on<event::http::client::connect>([](http::ClientResponse* response, net::Socket* socket, const Buffer& head){
			std::cout << "[req] on connect" << std::endl;
		});
		req->on<event::http::client::upgrade>([](http::ClientResponse* response, net::Socket* socket, const Buffer& head){
		...
		req->on<native::event::http::client::response>([](http::ClientResponse* res){
